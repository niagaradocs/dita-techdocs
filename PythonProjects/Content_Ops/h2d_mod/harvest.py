...
def _gather_concept_blocks(container: Tag, graphic_rel_val: str, img_scale: Optional[str], allow_sections: bool = False) -> List[str]:
    ...
    """
    from bs4 import BeautifulSoup  # import here to ensure availability
    ...
    while i < len(children):
        ...
        if name == "li":
            ...
            if li_items:
                try:
                    # Ensure we have a valid soup context that supports new_tag()
                    # Ensure we have a valid soup context that supports callable new_tag()
                    if hasattr(container, "new_tag") and callable(getattr(container, "new_tag", None)):
                        soup_ctx = container
                    else:
                        # Fallback: lightweight soup just for tag creation
                        from bs4 import BeautifulSoup
                        soup_ctx = BeautifulSoup("", "html.parser")

                    fake_ul = soup_ctx.new_tag("ul")
                    ...
                    print(f"Warning: failed to wrap orphan <li> block: {e}")
