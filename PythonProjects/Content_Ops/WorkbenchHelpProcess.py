"""
CURRENT VERSION 1.2 Active, 3 July 2025 - added fix for internal xref links
...
class HelpSystemProcessor:
    ...
    def update_links_in_html(self, target_root_folder):
        ...
        """Update links in HTML files to match the new file structure, including fragment identifiers."""
        ...
        for dirpath, _, filenames in os.walk(target_root_folder):
            ...
            for filename in filenames:
                ...
                if filename.endswith('.html'):
                    ...
                    # Update all tags with href or src attributes

                    for tag in soup.find_all(href=True):

                        tag['href'] = self.update_links(tag['href'])

                    for tag in soup.find_all(src=True):

                        tag['src'] = self.update_links(tag['src'])
    ...
    def update_links(self, link):
        """Update a link based on the file name mapping, preserving fragment identifiers."""
        if link.lower().endswith(('.png', '.jpg', '.jpeg', '.gif')):
            return f'graphics/{os.path.basename(link)}'

        if '.html' in link.lower():
            if '#' in link:
                path_part, fragment = link.split('#', 1)
                base_name = os.path.basename(path_part)
                new_base = self.file_name_mapping.get(base_name, base_name)
                return f'{new_base}#{fragment}'
            else:
