#!/usr/bin/env python3
#!/usr/local/bin
# This file is working great as of May 11, 2021!
import re
import os
import shutil
import subprocess
...
for doc in (docs):
    ...
    libs = f"c:/niagara/techdocsdev/docs/{doc}/{doc}-doc/build/libs/"
    jars = f"c:/niagara/jarHolder"
    ...
    j = re.compile('doc.*.jar')  # javascript and css links
...
#======== cleanup part 1: delete files and folders ============
    ...
    print('All ' + doc + ' files cleaned. Total docs cleaned =' + str(i))
    #---------------------- Run Gradlew -------------------------
    ...
    if os.path.exists(libs):
        os.rename(os.path.join(libs, j), os.path.join(dst, '{doc}'))
    else:
